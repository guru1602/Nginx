...
smoke_assert_code() {
    ...
    if [[ $CODE == $EXPECTED ]]; then
        ...
        _smoke_fail "$EXPECTED Response code; got $CODE"
    fi
}
...
smoke_assert_code_ok() {
    ...
    if [[ $CODE == 2* ]]; then
        ...
        _smoke_fail "2xx Response code; got $CODE"
    fi
}
...
## Curl helpers
_curl() {
    local opt=(--cookie $SMOKE_CURL_COOKIE_JAR --cookie-jar $SMOKE_CURL_COOKIE_JAR --location --dump-header $SMOKE_CURL_HEADERS --silent -k)
...
_curl_get() {
    ...
    _smoke_print_url "($SMOKE_HEADERS) $SMOKE_URL"
